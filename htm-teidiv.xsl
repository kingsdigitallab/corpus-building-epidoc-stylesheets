      ...
      <xsl:if test="not(t:head)">
        <xsl:choose>
          ...
          <xsl:when test="@type = 'translation'">
              <h2>
                <xsl:value-of
                  select="/t:TEI/t:teiHeader/t:profileDesc/t:langUsage/t:language[@ident = current()/@xml:lang]"/>
                <xsl:text> </xsl:text>
                <xsl:value-of select="@type"/>
              </h2>
              <p class="translation-author">
                <xsl:for-each
                  select="/t:TEI/t:teiHeader/t:fileDesc/t:titleStmt/t:respStmt/t:name[@xml:id = substring-after(current()/@resp, '#')]">
                  <xsl:choose>
                    <xsl:when test="@ref">
                      <a href="{@ref}">
                        <xsl:value-of select="."/>
