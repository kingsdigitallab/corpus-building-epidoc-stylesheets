...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template match="t:unclear">
      ...
      <xsl:param name="text-content">
         <xsl:choose>
            <xsl:when test="ancestor::t:orig[not(ancestor::t:choice)]">
               <xsl:value-of select="
                     upper-case(translate(normalize-unicode(translate(., '&#x03f2;', '&#x03f9;'), 'NFD'),
                     '&#x0300;&#x0301;&#x0308;&#x0313;&#x0314;&#x0342;&#x0345;', ''))"
               />
            </xsl:when>
      ...
      <xsl:variable name="hasPunicLang"
         select="ancestor::t:div[@type = 'edition']/@xml:lang = 'xpu-Latn'"/>
