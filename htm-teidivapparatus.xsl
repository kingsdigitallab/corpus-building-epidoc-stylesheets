...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-prefixes="#all" version="2.0">
  ...
  <xsl:template name="sources">
    ...
    <xsl:variable name="final_printing_sources">
      <xsl:for-each select="$sources/t:ref">
        ...
        <xsl:if test="
            not($n_authors_with_same_name_in_local_bib_and_current_sources = $n_authors_with_same_name_in_current_sources)
            or $first_occurrence_of_this_author_in_sources">

          <t:ref>
            <xsl:sequence select="./t:name"/>
            <xsl:if test="
                $n_authors_with_same_name_in_local_bib != 1
  ...
  <xsl:template match="t:div[@type = 'apparatus']//t:app">
    ...
    <span>
      ...
      <xsl:if
        test="@loc and (not(preceding-sibling::t:app) or not(preceding-sibling::t:app/@loc) or @loc != preceding-sibling::t:app[1]/@loc)">
  ...
  <xsl:template
    match="t:div[@type = 'apparatus']//t:rdg/@resp | t:div[@type = 'apparatus']//t:lem/@resp">
    <xsl:param name="parm-external-app-style" tunnel="yes" required="no"/>
    <xsl:param name="parm-edn-structure" tunnel="yes" required="no"/>
