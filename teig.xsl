...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-prefixes="t EDF" version="2.0">
   ...
   <xsl:template match="t:g">
      ...
      <xsl:choose>
         ...
         <xsl:when test="$parm-leiden-style = ('medcyprus')">
            ...
            <xsl:choose>
               <xsl:when
                  test="doc-available($symbols-al) = fn:true() and document($symbols-al)//t:glyph[@xml:id = $symbol]">
                  <xsl:variable name="symbol-id"
                     select="document($symbols-al)//t:glyph[@xml:id = $symbol]"/>
                  <xsl:choose>
                     <xsl:when
                        test="$parm-edition-type = 'diplomatic' and $symbol-id//t:mapping[@type = 'glyph-display']">
         ...
         <xsl:otherwise>
            <xsl:call-template name="chardecl">
               <xsl:with-param name="g" select="."/>
               <xsl:with-param name="parm-chardecl" select="$chardecl"/>
            </xsl:call-template>
         </xsl:otherwise>
      </xsl:choose>
   </xsl:template>
   ...
   <xsl:template name="chardecl">
      ...
      <xsl:param name="parm-glyph-variant" tunnel="yes" required="no"/>
      <xsl:param name="g"/>
      <xsl:param name="parm-chardecl"/>

      <!--      stores the chardecl: if locally included, uses that one, otherways uses the common one, i.e. local definitions override -->
      <xsl:variable name="chardecl" select="
            if ($parm-chardecl) then
               $parm-chardecl
            else
               if (//t:charDecl) then
                  //t:charDecl
               else
                  (if (doc('charDecl.xml')) then
                     doc('charDecl.xml')
                  else
                     doc(concat('file:', system-property('user.dir'), '/webapps/ROOT/kiln/stylesheets/epidoc/charDecl.xml')))"/>
      ...
      <xsl:choose>
            ...
            <xsl:for-each select="$chardecl//t:glyph[@xml:id = $glyphID]">
               <xsl:choose>
                  <xsl:when test="$parm-edition-type = 'diplomatic'">
                     <xsl:variable name="glyphDiplomatic"
                        select="concat($parm-glyph-variant, '-diplomatic')"/>
                     <xsl:choose>
         ...
         <xsl:when test="@ref">
            ...
            <xsl:variable name="externalCharDecl" select="
                  if ($parm-leiden-style = 'medcyprus') then
                     concat('file:', system-property('user.dir'), '/webapps/ROOT/content/xml/authority/', substring-before($parsedRef, '#'))
                  else
                     substring-before($parsedRef, '#')"/>
            <xsl:choose>
               <xsl:when test="doc-available($externalCharDecl)">
                  ...
                  <xsl:choose>
                     ...
                     <xsl:otherwise>
                        ...
                        <xsl:value-of select="
                              if (contains($parsedRef, '#')) then
                                 substring-after($parsedRef, '#')
                              else
                                 if (contains($parsedRef, ':')) then
                                    substring-after($parsedRef, ':')
                                 else
               ...
               <xsl:otherwise>
                  <xsl:message>The XSLT could not locate <xsl:value-of select="@ref"
                     />.</xsl:message>
                  <xsl:value-of select="@ref"/>
               </xsl:otherwise>
            </xsl:choose>
