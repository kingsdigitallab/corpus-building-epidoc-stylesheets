...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="t:lb">
        ...
        <xsl:param
            name="parm-edition-type" tunnel="yes" required="no" />
        <xsl:param
            name="parm-leiden-style" tunnel="yes" required="no" />
        <xsl:param name="parm-line-inc"
            tunnel="yes" required="no" />
        <xsl:param name="parm-verse-lines" tunnel="yes"
            required="no" />
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when test="$parm-leiden-style = 'eagletxt'">
                            ...
                            <xsl:if
                                ...
                                <xsl:choose>
                                    <xsl:when
                                        test="not(@break = 'no' or ancestor::t:w | ancestor::t:name | ancestor::t:placeName | ancestor::t:geogName)">
                                        <xsl:text> / </xsl:text>
                                    </xsl:when>
                                    <xsl:when
                                        test="@break = 'no' or ancestor::t:w | ancestor::t:name | ancestor::t:placeName | ancestor::t:geogName">
                        ...
                        <xsl:when
                            test="($parm-leiden-style = 'ddbdp' and ((not(ancestor::*[name() = 'TEI'])) or $location = 'apparatus')) or ($parm-edn-structure = 'inslib' and ancestor::t:div[@type = 'apparatus'])" />
                        ...
                        <!--  *unless* previous line ends with space / g / supplied[reason=lost]  (if
                        not MedCyprus project) -->
                        <!-- in which case the hyphen will be inserted before the space/g r final
                        ']' of supplied
                            (tested by EDF:f-wwrap in functions.xsl, which is called by teisupplied.xsl, teig.xsl and
                        teispace.xsl) -->
                        <xsl:when
                            test="
                        ...
                        <!-- in which case the hypen will be inserted before the paragraphos by code
                        in htm-teimilestone.xsl -->
                        <xsl:when
                ...
                <!-- Show arrows based on the immediately preceding lb in the same ab; avoid numeric
                casts on $line/@n -->
                <xsl:variable
                    name="this" select="." />
                <xsl:variable name="prevLb"
                    select="(preceding::t:lb[ancestor::t:ab[1] is $this/ancestor::t:ab[1]])[1]" />
                <xsl:if
                    test="$prevLb">
                    <xsl:choose>
                        <xsl:when test="$prevLb/@style = 'text-direction:l-to-r'">
                            <span class="tei text-direction l-to-r">
                                <xsl:text>&#xa0;&#xa0;→</xsl:text>
                            </span>
                        </xsl:when>
                        <xsl:when test="$prevLb/@style = 'text-direction:r-to-l'">
                ...
                <xsl:if
                    test="$parm-edn-structure = 'inslib' and ancestor::t:l/preceding::t:l[1]//t:lb[last()][@rend = 'left-to-right']">
                    <xsl:text>&#xa0;&#xa0;→</xsl:text>
                </xsl:if>
                <xsl:if
                    test="$parm-edn-structure = 'inslib' and ancestor::t:l/preceding::t:l[1]//t:lb[last()][@rend = 'right-to-left']">
                    <xsl:text>&#xa0;&#xa0;←</xsl:text>
                </xsl:if>
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="self::t:lb is ancestor::t:div[1]/t:*[child::t:lb][1]/t:lb[1]">
                        ...
                        <!-- for the first lb in a div, create an empty anchor instead of a
                        line-break -->
                    </xsl:when>
                    <!-- Commented out, causes incorrect formatting. '|' should only appear in
                    apparatus. See: https://github.com/DCLP/dclpxsltbox/issues/119 TODO:
                    Investigate. Canceled comment (HAC)
                    -->
                    <xsl:when
                        test="
    ...
    <xsl:template name="margin-num">
        ...
        <xsl:choose>
            <!-- don't print marginal line number inside tags that are relegated to the apparatus
            (ddbdp) -->
            <xsl:when test="$parm-leiden-style = 'eagletxt'" />
            <xsl:when
                test="
